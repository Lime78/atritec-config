...
import { saveAs } from 'file-saver';
  ...
  const Landing = ({ trackData }) => {
    ...
    const cirrusParagrams = [
      ...
      { name: 'move_to_nas', type: 'bBool', default: 5845, desc: "EPSG Code for processing pointcloud data ( pointcloud is exported in this CRS )" },
  ...
  useEffect(() => {
    ...
    const generateCirrusInputs = (cirrusParagrams) => {
      const inputs = [];

      cirrusParagrams.forEach((paragram) => { 
        if (paragram.type === '') {
          inputs.push({ value: '', placeholder: paragram.name });
        } else {
          inputs.push({ value: '', placeholder: paragram.name });
        }
      });
    ...
    setCirrusInputs(generateCirrusInputs(cirrusParagrams));
  }, [cirrusParagrams]);
